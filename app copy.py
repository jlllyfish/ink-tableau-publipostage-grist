            ...
            if filename.endswith('.json'):
                ...
                try:
                    with open(filepath, 'r', encoding='utf-8') as f:
                        ...
                        # CORRECTION : Chercher dans customization
                        customization = config.get('customization', {})
                        table = config.get('table', {})
                        
                        configs.append({
                            'filename': filename,
                            'name': config.get('config_name', 'Sans nom'),
                            'table_id': table.get('table_id', 'N/A'),
                            'created_at': config.get('created_at', None),
                            'has_logo': bool(customization.get('logo_path')),
                            'has_signature': bool(customization.get('signature_path'))
                        })
                except Exception as e:
                    print(f"Erreur lecture config {filename}: {e}")
                    continue
...
def save_configuration():
    ...
    try:
        ...
        if not config_name:
            return jsonify({'error': 'Nom de configuration requis'}), 400
        
        # Créer un nom de fichier sécurisé
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        filename = secure_filename(f"{config_name}_{timestamp}.json")
        filepath = os.path.join(CONFIGS_FOLDER, filename)
...
def delete_configuration(filename):
    ...
    try:
        ...
        # Lire la config pour récupérer les fichiers associés (logo, signature)
        try:
            with open(filepath, 'r', encoding='utf-8') as f:
                config = json.load(f)
            ...
            if 'customization' in config:
                # Supprimer le logo si présent et dans le dossier temporaire
                logo_path = config['customization'].get('logo_path')
                if logo_path and os.path.exists(logo_path) and logo_path.startswith(LOGO_UPLOAD_FOLDER):
                    try:
                        os.remove(logo_path)
                        print(f"Logo supprimé: {logo_path}")
                    except:
                        pass
                
                # Supprimer la signature si présente
                signature_path = config['customization'].get('signature_path')
                if signature_path and os.path.exists(signature_path) and signature_path.startswith(Config.UPLOAD_FOLDER):
                    try:
                        os.remove(signature_path)
                        print(f"Signature supprimée: {signature_path}")
                    except:
                        pass
        except:
            pass
